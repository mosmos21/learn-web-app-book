...
export const whereByCategoryId = async (categoryId: number): Promise<Model.TaskWithCategory[]> => {
  const columns = ["tasks.id", "c.name as categoryName", "title", "content", "status"];
  const [rows] = await pool.query<(RowDataPacket & {
    id: number;
    categoryName: string;
    title: string;
...
export const whereByUserId = async (userId: number): Promise<Model.TaskWithCategory[]> => {
  const columns = ["tasks.id", "c.id as categoryId", "c.name as categoryName", "title", "content", "status"];
  const [rows] = await pool.query<(RowDataPacket & {
    id: number;
    categoryId: number;
    categoryName: string;
